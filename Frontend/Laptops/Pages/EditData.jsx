...
function EditData() {
    ...
    useEffect(()=>{
        axios.get(`https://five3-beyond-pricing-laptops.onrender.com/Laptops/`+id)
        .then(res=>{
            setName(res.data.Name)
            setImage(res.data.image)
            setRam(res.data.RAM_GB)
            setRom(res.data.ROM_GB)
            setRamType(res.data.RAM_Type)
            setRomType(res.data.ROM_Type)
            setBattery(res.data.Battery_hrs)
            setOperating(res.data.Operating_System)
    ...
    const Submit=(e)=>{
        e.preventDefault()
        axios.put(`https://five3-beyond-pricing-laptops.onrender.com/UpdateUser/`+id,{Name, image, RAM_GB, ROM_GB, RAM_Type, ROM_Type, Battery_hrs, Operating_System, Price, Review})
        .then(res=>{
            Navigate('/data')
            console.log(res);
        })
